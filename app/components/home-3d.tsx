...
const AudioVisualizer = ({ scale = 1 }: { scale?: number }) => {
  ...
  useFrame((state) => {
    ...
    barsRef.current.forEach((bar, i) => {
      if (bar) {
        // Animasi scale sederhana (sinewave)
  ...
  return (
    <group scale={scale}>
      {[...Array(5)].map((_, i) => (
        ...
        >
          {/* Geometri kotak sangat ringan */}
          <boxGeometry args={[0.3, 2.5, 0.3]} />
          {/* Gunakan MeshStandardMaterial tanpa fitur berat */}
    ...
    return (
        <Float speed={1.5} rotationIntensity={0.5} floatIntensity={0.5}>
            <mesh ref={meshRef} position={position} rotation={[1, 0, 0]}>
                {/* Kurangi segmen torus: 16, 32 (sebelumnya 16, 64) */}
                <torusGeometry args={[1.4, 0.05, 16, 32]} />
                
                {/* LOGIKA MATERIAL:
                    Mobile: Pakai Standard Material (Transparan biasa) -> Ringan
                    Desktop: Pakai Transmission (Kaca Bias) -> Berat tapi cantik
                */}
                {isMobile ? (
                    <meshStandardMaterial color="white" opacity={0.3} transparent roughness={0.2} />
                ) : (
                    <MeshTransmissionMaterial 
                        backside backsideThickness={1} samples={2} resolution={128} // Resolusi & Sample Rendah
...
export const ParallaxShapes = () => {
  ...
  useFrame((state, delta) => {
    ...
    // Animasi Visualizer (Parallax)
    if (visualizerRef.current) {
        const targetY = isMobile ? -2.5 + (offset * 6) : -0.5 + (offset * 3);
  ...
  return (
    <group>
      ...
      <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
        <mesh ref={donutRef} position={[2, 2, 0]} castShadow={!isMobile}> {/* Matikan shadow di HP */}
          <torusGeometry args={isMobile ? [0.3, 0.12, 16, 32] : [0.7, 0.25, 24, 48]} />
          <meshStandardMaterial color={THEME.accent} roughness={0.4} metalness={0.5} />
        </mesh>
      </Float>

      {/* 2. KAPSUL (Optimized Material) */}
      <Float speed={3} rotationIntensity={0.5} floatIntensity={0.5}>
        <mesh ref={capsuleRef} position={[-2, -1, 0]} rotation={[0,0,0.5]} castShadow={!isMobile}>
          ...
          {/* Mobile pakai material biasa, Desktop pakai kaca */}
          {isMobile ? (
             <meshStandardMaterial color={THEME.primary} opacity={0.7} transparent roughness={0.1} />
          ) : (
             <MeshTransmissionMaterial 
                backside backsideThickness={1} samples={2} resolution={128} // Resolusi rendah
      ...
      <Float speed={1.5} rotationIntensity={1} floatIntensity={1}>
         <mesh ref={sphereRef} position={[3, 0, -2]} scale={isMobile ? 0.5 : 1}>
            <sphereGeometry args={[0.8, 24, 24]} /> {/* Segmen dikurangi */}
...
export const Lighting = () => (
  <>
    ...
    {/* Matikan castShadow global untuk performa */}
    <spotLight 
        position={[5, 10, 5]} 
    ...
    {/* Environment resolusi rendah cukup untuk refleksi */}
    <Environment preset="city" blur={1} />
    
    {/* HAPUS SOFTSHADOWS - Ini fitur terberat di drei, tidak perlu untuk background */}
  </>
);