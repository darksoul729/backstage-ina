...
export default function LayananPage() {
  ...
  return (
    // Background Hitam Eggplant (#1a0518) Solid tanpa dekorasi tambahan
    <div className="min-h-screen bg-[#1a0518] text-[#f8fafc] px-6 md:px-24 py-32 relative overflow-x-hidden selection:bg-[#9DB035] selection:text-[#3F1D38]">
      ...
      <div className="fixed top-6 left-6 md:top-10 md:left-10 z-50">
        <Link 
            href="/" 
            className="group flex items-center gap-3 pl-2 pr-6 py-2 bg-[#1a0518]/90 border border-white/10 rounded-full transition-all duration-300 hover:border-[#9DB035]"
        >
            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/5 group-hover:bg-[#9DB035] group-hover:border-[#9DB035] transition-all duration-300">
                <ArrowRight className="text-white w-4 h-4 rotate-180 group-hover:text-[#3F1D38] transition-colors duration-300 group-hover:-translate-x-1" />
            </div>
      ...
      {/* (BAGIAN BACKGROUND GLOW/BLUR DIHAPUS AGAR POLOS) */}
      ...
      <div className="mb-20 text-center relative z-10 mt-10">
        <h1 className="text-5xl md:text-8xl font-black mb-6 tracking-tighter text-white">
          Spesialisasi <span className="text-[#9DB035]">Teknis.</span>
        </h1>
          ...
          return (
            <motion.div 
              ...
              className={`relative p-8 md:p-10 rounded-[2.5rem] border transition-all cursor-pointer overflow-hidden duration-500
                ${isActive 
                    ? 'bg-[#1a0518] border-[#9DB035] shadow-[0_0_50px_rgba(157,176,53,0.15)]' 
                    : 'bg-white/[0.02] border-white/10 hover:border-[#9DB035]/50'
                }
              `}
            >
                ...
                <motion.div layout className="relative w-full mb-8">
                    {isActive ? (
                        ...
                        <motion.div 
                            ...
                            className="h-64 w-full rounded-2xl overflow-hidden bg-black/20 border border-[#9DB035]/20"
                        >
                        ...
                        <motion.div 
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="w-20 h-20 rounded-2xl bg-[#9DB035] text-[#3F1D38] flex items-center justify-center shadow-lg"
                        >
                ...
                <motion.div layout>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className={`text-3xl font-bold transition-colors ${isActive ? 'text-[#9DB035]' : 'text-white'}`}>
                            {item.title}
                        </h3>
                        {/* Indikator Status */}
                        <div className={`p-2 rounded-full border transition-colors duration-300 ${isActive ? 'border-[#9DB035] text-[#9DB035] bg-[#9DB035]/10' : 'border-white/10 text-slate-500'}`}>
